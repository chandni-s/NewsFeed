...
class TweeStreamer(twython.TwythonStreamer):
    ...
    def __init__(self, app_key, app_secret, oauth_token, oauth_token_secret, database):
        """
        """
        super(TweeStreamer, self).__init__(app_key, app_secret, oauth_token, oauth_token_secret)
    ...
    def on_success(self, data):
        ...
        ???(Cannot sure whether need this, the returned list might cannot be
        caught and used.)
        """       
        ...
        t_url = 'https://twitter.com/' +  t_author + '/' \
            + data['created_at'] + '/'
        content = data['text']
        
        # The article title is whole content of tweet
        t_article = Article(url=t_url, title=content, 
                            date=data['created_at'], author= t_author)

        add_t =self.database.add_article(t_article)
...
if __name__ == "__main__":
    ...
    db = Database('test_twitter')
    db.create_tables()   
  
    set_up_streamer(consumer_key, consumer_secret, 
                            access_token, access_token_secret, 'Israel',db)   
    
    
    
    '''
    db.add_source(Source("cnn.com")) 
    ...
    tweet_1 = db.add_article(Article("twitter.com/author1/20141124/"
                                      "new-york-murder-suicide/index.html?hpt=ju_c1", 
                                 tweet_html1, 
                                 "2014/11/24", 
                                 "Author1"))
    
    test_search = search_references(db, tweet_html1, 1)
    refs = db.get_references(1).all()
    print refs
    print len(refs)
    '''
