...
from plot2d import plot_lines, plot_bars
...
def db_add_keyword():
    ...
    keyword = Keyword(
        ...
        request.args.get('name', u'', type=unicode))
...
def db_add_reference():
    ...
    """
    ref = Reference(
        child_id=request.args.get('child_id', None, type=int),
    ...
    if db.add_reference(ref):
        ...
        return Response(result=False,
                        msg="A reference cannot be made to the "
                            "same source as the original article, "
                            "or to an article from the same source.").to_json()


@app.route('/db/add_watch')
def db_add_watch():
    """(None) -> str
    Adds a URL to watch to the database.
    """
    try:
        watch = Watch(url=request.args.get('url', '', type=str))
    except (TldDomainNotFound, TldBadUrl):
        return Response(result=False,
                        msg="Watch URL was not valid.").to_json()

    db = get_db()
    if db.add_watch(watch):
        return Response(result=True, msg="Watch added.").to_json()
    else:
        return Response(result=False, msg="Watch already exists in the "
                                          "database.").to_json()
...
@app.route('/db/delete_watch')
def db_delete_watch():
    """(None) -> str
    Deletes a watch, given by id.
    """
    id = request.args.get('id', None, type=int)
    db = get_db()
    if id > 0 and db.delete_by_id(Watch, id):
        return Response(result=True, msg="Watch deleted.").to_json()
    else:
        return Response(result=False, msg="No watch selected.").to_json()
...
def db_get_articles():
    ...
    query = db.get_articles(
        url=request.args.get('url', u'', type=unicode),
        title=request.args.get('title', u'', type=unicode),
        author=request.args.get('author', u'', type=unicode),
        tags=request.args.get('tags', u'', type=unicode),
        date_start=request.args.get('date_start', u'', type=unicode),
        date_end=request.args.get('date_end', u'', type=unicode),
        query=request.args.get('query', u'', type=unicode))
...
def db_get_sources():
    ...
    db = get_db()
    query = db.get_sources(request.args.get('url', u'', type=unicode))
...
@app.route('/db/get_watches')
def db_get_watches():
    """(None) -> str
    Gets the watches that match the given query.
    """
    # Get the watches query for the given parameters.
    db = get_db()
    query = db.get_watches(request.args.get('url', u'', type=unicode))

    # Get the total number of sources in the filtered query.
    total = query.count()

    # Get the sources from the query.
    watches = query.subset(request.args.get('offset', 0, type=int),
                           request.args.get('length', 10, type=int),
                           by='url')

    # Give the watches, and the filtered count to the user interface.
    return Response(result=True,
                    data=watches,
                    params={'total': total}).to_json()
...
def db_modify_article():
    ...
    try:
        article = Article(
            request.args.get('url', u'', type=unicode),
            request.args.get('title', u'', type=unicode),
            request.args.get('date', u'', type=unicode),
            request.args.get('author', u'', type=unicode),
            request.args.get('tags', u'', type=unicode))
...
def db_modify_source():
    ...
    """
    try:
        source = Source(request.args.get('url', u'', type=unicode))
...
@app.route('/viz/draw_ref_in_article')
def viz_draw_ref_in_article():
    """(None) -> str
    Generates a visualization that shows all sources that appear
    as references inside an article, given by id.
    """
    # Get the article id.
    id = request.args.get('article', -1, type=int)
    if id <= 0:
        return Response(result=False,
                        msg="No article given.").to_json()

    # Generate the graph.
    filename = time.strftime("aref-%Y%m%d-%H%M%S.png")
    path = os.path.join("static", "img", filename)
    db = get_db()

    try:
        draw_references_in_article(db, path, id)
    except Exception as e:
        print e
        return Response(result=False,
                        msg="An error occurred while generating the "
                        "visualization. Try again.").to_json()

    return Response(result=True,
                    data={'filename': '/img/%s' % filename},
                    msg="Visualization generated.").to_json()


@app.route('/viz/draw_ref_from_source')
def viz_draw_ref_from_source():
    """(None) -> str
    Generates a visualization that shows all references made from a source,
    given a source id.
    """
    # Get the article id.
    id = request.args.get('source', -1, type=int)
    if id <= 0:
        return Response(result=False,
                        msg="No source given.").to_json()

    # Generate the graph.
    filename = time.strftime("s-ref-from-%Y%m%d-%H%M%S.png")
    path = os.path.join("static", "img", filename)
    db = get_db()

    try:
        draw_references_from_source(db, path, id)
    except Exception as e:
        print e
        return Response(result=False,
                        msg="An error occurred while generating the "
...
@app.route('/viz/draw_ref_to_source')
def viz_draw_ref_to_source():
    """(None) -> str
    Generates a visualization that shows all references made to a source, given
    a source id.
    """
    # Get the article id.
    id = request.args.get('source', -1, type=int)
    ...
    # Generate the graph.
    filename = time.strftime("s-ref-to-%Y%m%d-%H%M%S.png")
    path = os.path.join("static", "img", filename)
    db = get_db()

    try:
        draw_references_to_source(db, path, id)
    except Exception as e:
        print e
        return Response(result=False,
                        msg="An error occurred while generating the "
...
@app.route('/viz/plot_bar')
def viz_plot_bar():
    """(None) -> str
    Generates a 2D bar plot visualization for a given list of source IDs.
    """
    # Get the list of source ids.
    ids = request.args.get('sources', '', type=str)
    if not ids:
        return Response(result=False,
                        msg="No sources given").to_json()
    ids = ids.split()

    # Generate the line plot, and return the image.
    filename = time.strftime("bar-%Y%m%d-%H%M%S.png")
    path = os.path.join("static", "img", filename)
    db = get_db()

    plot_bars(db, path, ids)
...
@app.route('/viz/plot_lines')
def viz_plot_lines():
    """(None) -> str
    Generates a 2D line plot visualization for a given list of source IDs.
    """
    # Get the list of source ids.
    ids = request.args.get('sources', '', type=str)
    if not ids:
    ...
    # Generate the line plot, and return the image.
    filename = time.strftime("line-%Y%m%d-%H%M%S.png")
    path = os.path.join("static", "img", filename)
    db = get_db()

    try:
        plot_lines(db, path, ids)
    except Exception as e:
        print e
        return Response(result=False,
                        msg="An error occurred while generating the "
...
def web_crawl_url():
    ...
    try:
        url = unquote(request.args.get('url', u'', type=unicode))
    except:
        return Response(result=False,
    ...
    recursive = request.args.get('recursive', u'', type=unicode)
    if recursive == 'true':
        depth = 3
    else:
        depth = 1
    ...
    # then it crawls up to 3 URLs deep (for a max of 7 articles).
    db = get_db()
...
@app.route('/web/crawl_watches')
def web_crawl_watches():
    """(None) -> str
    Crawls the watchlist.
    """
    thread.start_new(update, ())
    return Response(result=True,
                    msg="Updating watches.").to_json()
...
def update():
    ...
    with app.app_context():
        print "[Watchlist] Starting update."

        # Get all watched pages from the database.
        db = get_db()
        watches = db.get_watches().all()

        print "[Watchlist] There are %d URLs in the watchlist." % len(watches)

        for watch in watches:
            print "[Watchlist] Updating URL: %s" % watch.url

            try:
                # Check the domain, and handle it accordingly.
                if watch.domain == "twitter.com":
                    # This is a twitter account.
                    pass
                elif watch.url.endswith(('.rss', '.xml')):
                    # This is an RSS feed.
                    rss.parse_rss(db, watch.url, depth=2)
                else:
                    # Try crawling the page as a site.
                    web.crawl_site(db, watch.url, depth=3)
            except Exception as e:
                print "[Watchlist] Error occurred: %s" % e

        print "[Watchlist] Finished updating."
...
if __name__ == '__main__':
    ...
    # Create the visualization storage directy.
    try:
        os.mkdir("static/img")
    except:
        pass
